...
class DArray<E> {
   ...
   */
    ...
    */
   public E elementAt(int index)throws IndexOutOfBoundsException{
...
****/ 
   ...
   //new object array
      this.buffer = (E[]) new Object[a.buffer.length];
      this.size = a.size;
      //copy contents
   ...
   public boolean equals(DArray <E> that) {
   //testing if true
      if (this.size == that.size) {
         for (int a = 0; a < size; a++) {
            if (buffer[a]!= that.buffer[a]) {
               return false;
            }
         }
         return true;//match
      ...
      if(index >= this.size){ //expand the dynamic array
        //handle the array expansion/grow here
         if(size>=buffer.length){
            //calls to rezise method using the capacity growth portion as param
   ...
   //changed my resize method!
   public void resize(int changeSize) {
      E[] adjust = (E[]) new Object[changeSize];// creates a new array to copy buffer elements 
      buffer = adjust;
         // copying elements
      for (int a = 0; a < size; a++) {
         adjust[a] = buffer[a];
